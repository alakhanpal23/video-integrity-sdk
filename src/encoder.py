...
try:
    from . import keys
    from .config import load_config, get_attack_preset
    from .utils import progress_wrapper, Timer, set_deterministic_seed, validate_video_file
except ImportError:
    import keys
    from config import load_config, get_attack_preset
    from utils import progress_wrapper, Timer, set_deterministic_seed, validate_video_file
...
def embed_video(input_path: str, output_path: str, secret: np.ndarray, crf: int = 23, preset: str = None):
    """
    ...
    with Timer("Video embedding"):
        ...
        frames = []
        pbar = progress_wrapper(range(total_frames), desc="Embedding watermark")
        ...
        for frame in progress_wrapper(frames, desc="Writing video"):
...
if __name__ == "__main__":
    ...
    embed_parser.add_argument("--preset", type=str, choices=["reencode_light", "reencode_heavy", "social"], help="Attack simulation preset")
    embed_parser.add_argument("--seed", type=int, help="Random seed for reproducibility")
    embed_parser.add_argument("--config", type=str, help="Config file path")
    ...
    if args.command == "embed":
        ...
        if preset:
            preset_params = get_attack_preset(preset)
            crf = preset_params.get('crf', crf)
            print(f"Using preset '{preset}': CRF={crf}")
            
        embed_video(in_vid, out_vid, secret, crf=crf, preset=preset)
